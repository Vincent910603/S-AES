<template>
  <div>
    ...
    <div v-if="encrypted !== null">
      <p>加密结果：{{ encrypted.toString(16) }}</p>
    </div>
    <div v-if="decrypted !== null">
      <p>解密结果：{{ decrypted.toString(16) }}</p>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    encrypt() {
      ...
      const plainText = parseInt(this.plainText, 16); // 将十六进制明文转换为整数
      this.encrypted = aes.encrypt(plainText);
      this.decrypted = null; // 重置解密结果
    },
    decrypt() {
      const key = parseInt(this.key, 16); // 将十六进制密钥转换为整数
      const aes = new SimplifiedAES(key);
      const cipherText = parseInt(this.encrypted, 16); // 将十六进制加密结果转换为整数
      this.decrypted = aes.decrypt(cipherText);
    },
  },
};
</script>